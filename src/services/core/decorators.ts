...
const filteredMetaData = (type: string) => (parameters: ParametersMetaData) =>
...
function responseAdapterFactory() {
  return (adapter: ResponseAdapterType) =>
    (target: any, propertyName: string, descriptor: any) => {
      const responseAdapterMetaData = Reflect.getOwnMetadata(
        responseAdapterMetadataKey,
        target,
        propertyName
      )
...
function methodFactory(method: HttpMethod) {
  return (url: string) =>
    <T>(target: T, propertyName: string, descriptor: any) => {
      descriptor.value = function (...args: any[]) {
        const parametersMetaData: ParametersMetaData[] =
          Reflect.getOwnMetadata(parameterMetadataKey, target, propertyName) ||
          []
        const responseAdapterMetaData = Reflect.getOwnMetadata(
          responseAdapterMetadataKey,
          target,
        ...
        const pathParameters: ParametersMetaData[] = parametersMetaData.filter(
        ...
        const queryParameters: ParametersMetaData[] = parametersMetaData.filter(
          filteredMetaData('Query')
        )
        const params = queryParameters.reduce(
          (previous, current) => ({
            ...previous,
            [current.name]: args[current.index]
          }),
          {}
        )

        const dataParameters: ParametersMetaData[] = parametersMetaData.filter(
          filteredMetaData('Data')
        )
        ...
        return (this as HttpService).request(
          { method, url: subUrl, params, data },
          responseAdapterMetaData
        )
      }
